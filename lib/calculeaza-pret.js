...
// FIX (gaură de integritate, audit 22 august 2026): fix-ul din 2026-07-22
// (vezi nota de mai sus) a eliminat DELIBERAT vechiul VMC ("Valoare Minimă
// Comandă") din formulă — corect pentru arhitectura de rețetă aditivă. Dar
// front-end-ul (mydarrin-produs.html) nu a fost aliniat: încă aplică local
// `Math.max(brut, 100)` și afișează clientului promisiunea unui preț minim
// — promisiune pe care server-ul, de atunci, nu o mai impunea deloc. Un
// client putea trimite direct către API orice sumă sub acel prag, ocolindu-l
// complet. Acesta NU e sistemul complet de "cost minim de intervenție
// configurabil per țară" (propus separat, neaprobat încă) — e strict
// închiderea găurii active, cu aceeași valoare deja promisă azi în UI.
const VMC_MINIM_COMANDA = 100;
...
async function calculeazaPret(input = {}) {
  ...
  const pret_final = Math.max(Math.round((subtotalCuTaxePlatforma + tva_suma) * 100) / 100, VMC_MINIM_COMANDA);
