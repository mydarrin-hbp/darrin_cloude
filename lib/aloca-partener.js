...
async function trimiteEmailCod(email, comanda) {
  ...
  try {
    ...
    const qrHtml = qrDataUrl
      ? `<p><img src="${qrDataUrl}" width="180" height="180" alt="Cod QR verificare" style="display:block"/></p><p style="font-size:12px;color:#666">Sau arată acest QR partenerului — îl poate scana direct, fără să tasteze codul.</p>`
      : '';
...
async function incearcaAlocarePartener(comandaId) {
  ...
  // neschimbat, exact ca înainte de acest fix. Notă onestă: verificarea NU
  // se potrivește pe cod_esco specific comenzii — funcția asta matchează
  // deja doar pe catalog_serviciu_id (linia 92-97 de mai sus), fără nicio
  // legătură comandă→ESCO existentă în acest flux; e o verificare de "are
  // echipă, măcar unul disponibil", nu "are pe cineva calificat exact pentru
  // asta" — matching-ul fin pe competență rămâne pentru o extindere separată.
  const idCandidatiZona = disponibili.map((r) => r.partener_id);
  const { data: angajatiEchipe } = await supabaseAdmin
    .from('partner_angajati')
    .select('partner_id, disponibil')
    .in('partner_id', idCandidatiZona)
    .eq('activ', true);

  const echipaIdx = {};
  (angajatiEchipe || []).forEach((a) => {
    if (!echipaIdx[a.partner_id]) echipaIdx[a.partner_id] = { total: 0, disponibili: 0 };
    echipaIdx[a.partner_id].total += 1;
    if (a.disponibil) echipaIdx[a.partner_id].disponibili += 1;
  });

  const partenerId = idCandidatiZona.find((id) => {
    const echipa = echipaIdx[id];
    return !echipa || echipa.disponibili > 0;
  });
