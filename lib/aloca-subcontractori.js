...
// D2b (25 august 2026, aprobat) — transportator/ajutor reutilizează
// categoria curier_utilitara (aceeași ca 'curier'), fără enum nou în DB —
// toate trei sunt variante ale aceleiași activități de transport/logistică.
const ROL_LA_TIP_PARTENER = {
  materiale: 'furnizor_materiale',
  rental_echipament: 'inchirieri_utilaje',
  curier: 'curier_utilitara',
  asigurare: 'asigurari',
  transportator: 'curier_utilitara',
  ajutor: 'curier_utilitara',
};
...
/**
 * Varianta 2 "Echipă" (24 august 2026) — găsește un partener pentru un rând
 * `rol_tip='specialist'`, care cere un cod ESCO specific (derivat din rețeta
 * tehnică a comenzii, distinct de esco-ul principal deja alocat prin
 * lib/aloca-partener.js). Doi candidați posibili:
 *  1. parteneri CU echipă declarată (partner_angajati), care au un membru
 *     activ+disponibil cu exact acest cod_esco;
 *  2. parteneri SOLO (fără echipă), cu un serviciu activ pe portofoliu al
 *     cărui catalog_servicii.cod_esco se potrivește.
 * Restul filtrelor (eligibilitate hard-stop + zonă/disponibilitate) sunt
 * identice cu gasesteActorPentruRol.
 * @param {object} comanda
 * @param {string} codEsco
 * @returns {Promise<string|null>}
 */
async function gasesteActorPentruEsco(comanda, codEsco) {
  if (!codEsco) return null;

  const { data: angajatiPotriviti } = await supabaseAdmin
    .from('partner_angajati')
    .select('partner_id')
    .eq('cod_esco', codEsco)
    .eq('activ', true)
    .eq('disponibil', true);
  const candidatiEchipa = (angajatiPotriviti || []).map((a) => a.partner_id);

  const { data: serviciiEsco } = await supabaseAdmin
    .from('catalog_servicii')
    .select('id')
    .eq('cod_esco', codEsco);
  const idServiciiEsco = (serviciiEsco || []).map((s) => s.id);

  let candidatiSolo = [];
  if (idServiciiEsco.length) {
    const { data: activi } = await supabaseAdmin
      .from('partener_servicii_active')
      .select('partener_id')
      .in('catalog_serviciu_id', idServiciiEsco)
      .eq('activ', true);
    candidatiSolo = (activi || []).map((a) => a.partener_id);
  }

  let candidati = [...new Set([...candidatiEchipa, ...candidatiSolo])];
  if (!candidati.length) return null;

  const { data: eligibili } = await supabaseAdmin
    .from('partners')
    .select('id')
    .in('id', candidati)
    .eq('status_verificare', 'approved')
    .eq('contract_semnat', true);
  candidati = (eligibili || []).map((p) => p.id);
  if (!candidati.length) return null;

  const { data: conturi } = await supabaseAdmin
    .from('partner_conturi_bancare')
    .select('partner_id')
    .in('partner_id', candidati)
    .eq('activ', true);
  candidati = [...new Set((conturi || []).map((c) => c.partner_id))];
  if (!candidati.length) return null;
 ...
 * @param {object} comanda
 * @param {Array<{rol_tip:string, actor_id:string|null, suma_neta_alocata:number, cod_esco_necesar?:string}>} randuri
 */
async function completeazaActorii(comanda, randuri) {
  for (const rand of randuri) {
    if (rand.actor_id || rand.rol_tip === 'manopera') continue;
    rand.actor_id = rand.rol_tip === 'specialist'
      ? await gasesteActorPentruEsco(comanda, rand.cod_esco_necesar)
      : await gasesteActorPentruRol(comanda, rand.rol_tip);
  }
  return randuri;
}

module.exports = { gasesteActorPentruRol, gasesteActorPentruEsco, completeazaActorii };
