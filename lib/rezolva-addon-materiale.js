...
async function rezolvaAddonMateriale(addonMateriale, tierComanda, taraCod) {
  ...
  for (const cerere of addonMateriale) {
    ...
    const { data: addonServ } = await supabaseAdmin
      ...
      .select('id, titlu, categorie, unitate_masura, status_public, catalog_niveluri(id, nivel, catalog_preturi(pret, tara_cod))')
      .eq('id', addonId)
    ...
    itemi.push({
      catalog_serviciu_id: addonId, titlu: addonServ.titlu, categorie: addonServ.categorie,
