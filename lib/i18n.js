...
const PRIMA_COMANDA = {
  ro: {
    ...
    corp: (nume, nr) => `
      <p>Mulțumim pentru prima ta comandă, ${nume}!</p>
      <p>Tranzacția a fost procesată în siguranță, iar fondurile sunt protejate prin mecanismul nostru de escrow până la finalizarea serviciului sau livrarea materialelor. Poți urmări statusul live în panoul tău de control.</p>
  ...
  en: {
    ...
    corp: (nume, nr) => `
      <p>Thank you for your first order, ${nume}!</p>
      <p>The transaction was processed securely, and your funds are protected through our escrow mechanism until the service is completed or the materials are delivered. You can track live status in your dashboard.</p>
  ...
  it: {
    ...
    corp: (nume, nr) => `
      <p>Grazie per il tuo primo ordine, ${nume}!</p>
      <p>La transazione è stata elaborata in sicurezza e i tuoi fondi sono protetti dal nostro meccanismo di escrow fino al completamento del servizio o alla consegna dei materiali. Puoi seguire lo stato in tempo reale nel tuo pannello.</p>
  ...
  fr: {
    ...
    corp: (nume, nr) => `
      <p>Merci pour votre première commande, ${nume} !</p>
      <p>La transaction a été traitée en toute sécurité, et vos fonds sont protégés par notre mécanisme de séquestre (escrow) jusqu'à la fin du service ou la livraison des matériaux. Vous pouvez suivre le statut en direct dans votre tableau de bord.</p>
  ...
  de: {
    ...
    corp: (nume, nr) => `
      <p>Danke für deine erste Bestellung, ${nume}!</p>
      <p>Die Transaktion wurde sicher verarbeitet, und dein Geld ist durch unseren Treuhand-Mechanismus (Escrow) geschützt, bis die Dienstleistung abgeschlossen oder die Materialien geliefert sind. Du kannst den Status live in deinem Dashboard verfolgen.</p>
  ...
  es: {
    ...
    corp: (nume, nr) => `
      <p>¡Gracias por tu primer pedido, ${nume}!</p>
      <p>La transacción se procesó de forma segura, y tus fondos están protegidos mediante nuestro mecanismo de depósito en garantía (escrow) hasta que el servicio se complete o se entreguen los materiales. Puedes seguir el estado en vivo en tu panel.</p>
  ...
  tr: {
    ...
    corp: (nume, nr) => `
      <p>İlk siparişin için teşekkürler, ${nume}!</p>
      <p>İşlem güvenli şekilde gerçekleştirildi ve paran, hizmet tamamlanana veya malzemeler teslim edilene kadar emanet (escrow) mekanizmamızla korunuyor. Durumu canlı olarak panelinden takip edebilirsin.</p>
