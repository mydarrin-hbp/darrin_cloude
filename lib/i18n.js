...
// ── Logo în emailuri (BRAND-01, 27 august 2026) ──────────────────────
// URL static, servit de api/branding/logo-image.js — respectă exact logo-ul
// uploadat de admin în backoffice (api/branding/upload.js, Vercel KV), sau
// logo-ul implicit real al platformei dacă nimic nu e uploadat încă
// (niciodată o imagine placeholder/ruptă). Injectat o singură dată, la
// nivelul celor 10 funcții renderEmail*, nu în fiecare din cele ~11 variante
// de limbă ale fiecăreia — un singur punct de întreținere.
const LOGO_HTML_EMAIL = `<p style="margin:0 0 18px"><img src="https://mydarrin.homebestpal.com/api/branding/logo-image" alt="My Darrin" style="max-width:160px;height:auto;display:block" /></p>`;
...
function renderEmailBunVenitPartener(limba, { nume, tipLabel }) {
  ...
  return {
    subiect,
    html: LOGO_HTML_EMAIL + corpFn(tipLabel),
  };
}
...
function renderEmailContAbandonat(limba, rol, { nume }) {
  ...
  return {
    ...
    html: LOGO_HTML_EMAIL + set.corp(numeAfisat),
  };
}
...
function renderEmailNewsletterConfirmare(limba, link) {
  ...
  return { subiect: l.subiect, html: LOGO_HTML_EMAIL + l.corp(link) };
}
...
function renderEmailNewsletterBunVenit(limba) {
  ...
  return { subiect: l.subiect, html: LOGO_HTML_EMAIL + l.corp() };
}
...
function renderEmailInvestitorBunVenit(limba) {
  ...
  return { subiect, html: LOGO_HTML_EMAIL + corpFn() };
}
...
function renderEmailBunVenitClient(limba, { nume }) {
  ...
  return { subiect: l.subiect(), html: LOGO_HTML_EMAIL + l.corp(numeAfisat) };
}
...
// Notă 2 (29 august 2026, cerere fondator, verificat live): link-ul "Vezi
// Detalii Comandă" ducea la dashboard-ul generic, fără nicio legătură cu
// comanda reală — clientul ajungea pe pagina implicită, nu la comanda lui.
// Extins cu `?comanda=<uuid>` (citit de mydarrin-dashboard-client.html,
// deschide automat drawer-ul real `openDrawer()`, deja existent). Adăugat și
// paragraful cerut despre proforma + contracte disponibile în dashboard —
// ambele afișate DOAR dacă există date reale (numarProforma), niciodată text
// gol/generic dacă emiterea proformei a eșuat.
function _liniaProformaSiContracte(limba, numarProforma) {
  if (!numarProforma) return '';
  const TEXTE = {
    ro: (nr) => `<p>Factura proformă #${nr} a fost emisă și o găsești în contul tău. Contractele semnate la lansarea comenzii sunt disponibile tot acolo, în dashboard.</p>`,
    en: (nr) => `<p>Proforma invoice #${nr} has been issued and is available in your account. The contracts you signed when placing the order are also available there, in your dashboard.</p>`,
    it: (nr) => `<p>La fattura proforma #${nr} è stata emessa ed è disponibile nel tuo account. I contratti firmati al momento dell'ordine sono disponibili anch'essi lì, nella tua dashboard.</p>`,
    fr: (nr) => `<p>La facture proforma #${nr} a été émise et est disponible dans votre compte. Les contrats signés lors de la commande y sont également disponibles, dans votre tableau de bord.</p>`,
    de: (nr) => `<p>Die Proforma-Rechnung #${nr} wurde ausgestellt und ist in Ihrem Konto verfügbar. Die bei der Bestellung unterzeichneten Verträge finden Sie ebenfalls dort, in Ihrem Dashboard.</p>`,
    es: (nr) => `<p>La factura proforma #${nr} ha sido emitida y está disponible en tu cuenta. Los contratos firmados al realizar el pedido también están disponibles allí, en tu panel.</p>`,
    tr: (nr) => `<p>#${nr} numaralı proforma fatura düzenlendi ve hesabınızda mevcuttur. Sipariş sırasında imzaladığınız sözleşmeler de orada, panelinizde bulunmaktadır.</p>`,
    bg: (nr) => `<p>Проформа фактура #${nr} беше издадена и е налична в профила ви. Договорите, подписани при пускането на поръчката, също са налични там, в таблото ви.</p>`,
    el: (nr) => `<p>Το προτιμολόγιο #${nr} έχει εκδοθεί και είναι διαθέσιμο στον λογαριασμό σας. Οι συμβάσεις που υπογράψατε κατά την παραγγελία είναι επίσης διαθέσιμες εκεί, στον πίνακά σας.</p>`,
    hu: (nr) => `<p>A(z) #${nr} számú proforma számla kiállításra került, és elérhető a fiókodban. A rendelés leadásakor aláírt szerződések szintén ott, az irányítópulton érhetők el.</p>`,
    sr: (nr) => `<p>Профактура #${nr} је издата и доступна је у вашем налогу. Уговори потписани приликом покретања поруџбине такође су доступни тамо, на контролној табли.</p>`,
  };
  const fn = TEXTE[limbaValida(limba)] || TEXTE[LIMBA_FALLBACK];
  return fn(numarProforma);
}
...
const PRIMA_COMANDA = {
  ro: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Vezi Detalii Comandă</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.ro}</p>
    `,
  },
  en: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">View Order Details</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.en}</p>
    `,
  },
  it: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Vedi i dettagli dell'ordine</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.it}</p>
    `,
  },
  fr: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Voir les détails de la commande</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.fr}</p>
    `,
  },
  de: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Bestelldetails ansehen</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.de}</p>
    `,
  },
  es: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Ver detalles del pedido</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.es}</p>
    `,
  },
  tr: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Sipariş Detaylarını Gör</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.tr}</p>
    `,
  },
  bg: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Виж детайлите на поръчката</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.bg}</p>
    `,
  },
  el: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Δείτε τις λεπτομέρειες της παραγγελίας</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.el}</p>
    `,
  },
  hu: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Rendelés részleteinek megtekintése</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.hu}</p>
    `,
  },
  sr: {
    ...
    corp: (_nume, nr, link) => `
      ...
      <p><a href="${link}" style="${BTN_STYLE}">Погледајте детаље поруџбине</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.sr}</p>
    `,
  },
...
// Comandă confirmată — orice comandă în afară de prima (30 august 2026,
// cerere fondator: "rezolvă și pe acesta" — gap semnalat anterior, comenzile
// 2+ nu trimiteau niciun email de confirmare). Identică structural cu
// PRIMA_COMANDA (link real spre comandă + proformă atașată/menționată +
// dezabonare) — diferă DOAR prin formulare ("comanda ta", nu "prima
// comandă"), ca să nu inducă în eroare un client care a mai comandat.
const COMANDA_CONFIRMATA = {
  ro: {
    subiect: (nr) => `Comanda ta #${nr} a fost înregistrată cu succes! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Mulțumim pentru comanda ta! Plata ta este securizată prin escrow până la finalizarea livrării și poți urmări statusul în timp real din contul tău.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Vezi Detalii Comandă</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.ro}</p>
    `,
  ...
  en: {
    subiect: (nr) => `Your order #${nr} has been successfully placed! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Thank you for your order! Your payment is secured via escrow until delivery is complete, and you can track the status in real-time from your account.</p>
      <p><a href="${link}" style="${BTN_STYLE}">View Order Details</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.en}</p>
    `,
  ...
  it: {
    subiect: (nr) => `Il tuo ordine #${nr} è stato registrato con successo! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Grazie per il tuo ordine! Il tuo pagamento è protetto tramite escrow fino al completamento della consegna e puoi monitorare lo stato in tempo reale dal tuo account.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Vedi i dettagli dell'ordine</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.it}</p>
    `,
  ...
  fr: {
    subiect: (nr) => `Votre commande #${nr} a été enregistrée avec succès ! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Merci pour votre commande ! Votre paiement est sécurisé par séquestre jusqu'à la fin de la livraison, et vous pouvez suivre le statut en temps réel depuis votre compte.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Voir les détails de la commande</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.fr}</p>
    `,
  ...
  de: {
    subiect: (nr) => `Deine Bestellung #${nr} wurde erfolgreich aufgegeben! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Vielen Dank für Ihre Bestellung! Ihre Zahlung ist bis zum Abschluss der Lieferung über ein Treuhandkonto abgesichert, und Sie können den Status in Echtzeit in Ihrem Konto verfolgen.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Bestelldetails ansehen</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.de}</p>
    `,
  ...
  es: {
    subiect: (nr) => `¡Tu pedido #${nr} se ha registrado con éxito! 🎉`,
    corp: (_nume, nr, link) => `
      <p>¡Gracias por tu pedido! Tu pago está protegido mediante depósito en garantía hasta que se complete la entrega, y puedes seguir el estado en tiempo real desde tu cuenta.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Ver detalles del pedido</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.es}</p>
    `,
  ...
  tr: {
    subiect: (nr) => `#${nr} numaralı siparişin başarıyla alındı! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Siparişiniz için teşekkür ederiz! Teslimat tamamlanana kadar ödemeniz emanet (escrow) sistemiyle güvence altındadır ve durumu hesabınızdan gerçek zamanlı olarak takip edebilirsiniz.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Sipariş Detaylarını Gör</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.tr}</p>
    `,
  ...
  bg: {
    subiect: (nr) => `Поръчката ви #${nr} беше регистрирана успешно! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Благодарим ви за поръчката! Плащането ви е защитено чрез ескроу до завършване на доставката и можете да следите статуса в реално време от профила си.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Виж детайлите на поръчката</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.bg}</p>
    `,
  ...
  el: {
    subiect: (nr) => `Η παραγγελία σας #${nr} καταχωρίστηκε με επιτυχία! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Ευχαριστούμε για την παραγγελία σας! Η πληρωμή σας είναι ασφαλισμένη μέσω escrow έως την ολοκλήρωση της παράδοσης και μπορείτε να παρακολουθείτε την κατάσταση σε πραγματικό χρόνο από τον λογαριασμό σας.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Δείτε τις λεπτομέρειες της παραγγελίας</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.el}</p>
    `,
  ...
  hu: {
    subiect: (nr) => `A(z) #${nr} rendelésed sikeresen rögzítve! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Köszönjük a rendelésed! A fizetésed letéti (escrow) számlán van biztonságban a teljesítésig, és a státuszt valós időben követheted a fiókodból.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Rendelés részleteinek megtekintése</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.hu}</p>
    `,
  ...
  sr: {
    subiect: (nr) => `Ваша поруџбина #${nr} је успешно регистрована! 🎉`,
    corp: (_nume, nr, link) => `
      <p>Хвала вам на поруџбини! Ваша уплата је обезбеђена путем ескроу система до завршетка испоруке, а статус можете пратити у реалном времену са свог налога.</p>
      <p><a href="${link}" style="${BTN_STYLE}">Погледајте детаље поруџбине</a></p>
      __PROFORMA__
      <p style="font-size:12px;color:#666">${FOOTER_DEZABONARE.sr}</p>
    `,
...
function renderEmailPrimaComanda(limba, { nume, numarComanda, comandaId, numarProforma }) {
  const l = (PRIMA_COMANDA[limbaValida(limba)] || PRIMA_COMANDA[LIMBA_FALLBACK]);
  const numeAfisat = nume || (limbaValida(limba) === 'ro' ? 'acolo' : 'there');
  const link = comandaId ? `${DASHBOARD_CLIENT_URL}?comanda=${encodeURIComponent(comandaId)}` : DASHBOARD_CLIENT_URL;
  const corp = l.corp(numeAfisat, numarComanda, link).replace('__PROFORMA__', _liniaProformaSiContracte(limba, numarProforma));
  return { subiect: l.subiect(numarComanda), html: LOGO_HTML_EMAIL + corp };
}

function renderEmailComandaConfirmata(limba, { nume, numarComanda, comandaId, numarProforma }) {
  const l = (COMANDA_CONFIRMATA[limbaValida(limba)] || COMANDA_CONFIRMATA[LIMBA_FALLBACK]);
  const numeAfisat = nume || (limbaValida(limba) === 'ro' ? 'acolo' : 'there');
  const link = comandaId ? `${DASHBOARD_CLIENT_URL}?comanda=${encodeURIComponent(comandaId)}` : DASHBOARD_CLIENT_URL;
  const corp = l.corp(numeAfisat, numarComanda, link).replace('__PROFORMA__', _liniaProformaSiContracte(limba, numarProforma));
  return { subiect: l.subiect(numarComanda), html: LOGO_HTML_EMAIL + corp };
...
function renderEmailCodPartenerServicii(limba, { nume, numarComanda, cod }) {
  ...
  return { subiect: subiectCodPartener(numarComanda, l), html: LOGO_HTML_EMAIL + set(_numeAfisatPartener(nume, l), numarComanda, cod) };
}
...
// ── Reminder profil incomplet (48h) + email de finalizare 100% (Etapa 2i,
// 27 august 2026) — „profil 100% complet" = cont bancar activ + cel puțin
// un serviciu NACE declarat, definiție confirmată explicit de fondator.
// 7 limbi (ro/en/it/fr/de/es/tr), consistent cu CONT_ABANDONAT — celelalte
// 4 limbi (bg/el/hu/sr) cad corect pe fallback EN, tiparul deja stabilit.
const REMINDER_PROFIL_INCOMPLET = {
  ro: {
    subiect: () => 'Finalizează-ți profilul de partener My Darrin',
    corp: (nume) => `
      <p>Salut ${nume},</p>
      <p>Contul tău de partener My Darrin nu e încă 100% complet — îți mai lipsește <strong>un cont bancar activ</strong> și/sau <strong>cel puțin un serviciu declarat</strong> în portofoliu. Fără acestea, nu poți primi comenzi alocate și nu poți semna contractul.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Finalizează profilul</a></p>
      <p style="font-size:12px;color:#666">Vei primi acest mesaj din nou peste 48 de ore dacă profilul rămâne incomplet.</p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.ro}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.ro}</p>
    `,
  },
  en: {
    subiect: () => 'Complete your My Darrin partner profile',
    corp: (nume) => `
      <p>Hi ${nume},</p>
      <p>Your My Darrin partner account isn't 100% complete yet — you're still missing <strong>an active bank account</strong> and/or <strong>at least one declared service</strong> in your portfolio. Without these, you can't receive assigned orders or sign your contract.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Complete your profile</a></p>
      <p style="font-size:12px;color:#666">You'll receive this reminder again in 48 hours if your profile is still incomplete.</p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.en}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.en}</p>
    `,
  },
  it: {
    subiect: () => 'Completa il tuo profilo partner My Darrin',
    corp: (nume) => `
      <p>Ciao ${nume},</p>
      <p>Il tuo account partner My Darrin non è ancora completo al 100% — manca ancora <strong>un conto bancario attivo</strong> e/o <strong>almeno un servizio dichiarato</strong> nel tuo portafoglio.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Completa il profilo</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.it}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.it}</p>
    `,
  },
  fr: {
    subiect: () => 'Complétez votre profil partenaire My Darrin',
    corp: (nume) => `
      <p>Bonjour ${nume},</p>
      <p>Votre compte partenaire My Darrin n'est pas encore complet à 100 % — il vous manque encore <strong>un compte bancaire actif</strong> et/ou <strong>au moins un service déclaré</strong> dans votre portefeuille.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Compléter le profil</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.fr}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.fr}</p>
    `,
  },
  de: {
    subiect: () => 'Vervollständige dein My Darrin Partnerprofil',
    corp: (nume) => `
      <p>Hallo ${nume},</p>
      <p>Dein My Darrin Partnerkonto ist noch nicht zu 100 % vollständig — es fehlt noch <strong>ein aktives Bankkonto</strong> und/oder <strong>mindestens eine deklarierte Dienstleistung</strong> in deinem Portfolio.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Profil vervollständigen</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.de}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.de}</p>
    `,
  },
  es: {
    subiect: () => 'Completa tu perfil de socio My Darrin',
    corp: (nume) => `
      <p>Hola ${nume},</p>
      <p>Tu cuenta de socio My Darrin aún no está completa al 100% — todavía te falta <strong>una cuenta bancaria activa</strong> y/o <strong>al menos un servicio declarado</strong> en tu portafolio.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Completar perfil</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.es}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.es}</p>
    `,
  },
  tr: {
    subiect: () => 'My Darrin partner profilinizi tamamlayın',
    corp: (nume) => `
      <p>Merhaba ${nume},</p>
      <p>My Darrin partner hesabınız henüz %100 tamamlanmadı — hâlâ <strong>aktif bir banka hesabı</strong> ve/veya portföyünüzde <strong>en az bir beyan edilmiş hizmet</strong> eksik.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Profili tamamla</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.tr}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.tr}</p>
    `,
  },
...
const PROFIL_PARTENER_COMPLET = {
  ro: {
    subiect: () => 'Profilul tău de partener este complet! 🎉',
    corp: (nume) => `
      <p>Felicitări ${nume},</p>
      <p>Profilul tău de partener My Darrin este acum 100% complet — ai un cont bancar activ și cel puțin un serviciu declarat în portofoliu. Poți primi comenzi alocate din platformă.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Mergi la dashboard</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.ro}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.ro}</p>
    `,
  },
  en: {
    subiect: () => 'Your partner profile is complete! 🎉',
    corp: (nume) => `
      <p>Congratulations ${nume},</p>
      <p>Your My Darrin partner profile is now 100% complete — you have an active bank account and at least one declared service in your portfolio. You can now receive orders assigned through the platform.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Go to dashboard</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.en}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.en}</p>
    `,
  },
  it: {
    subiect: () => 'Il tuo profilo partner è completo! 🎉',
    corp: (nume) => `
      <p>Congratulazioni ${nume},</p>
      <p>Il tuo profilo partner My Darrin è ora completo al 100%. Puoi ricevere ordini assegnati dalla piattaforma.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Vai alla dashboard</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.it}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.it}</p>
    `,
  },
  fr: {
    subiect: () => 'Votre profil partenaire est complet ! 🎉',
    corp: (nume) => `
      <p>Félicitations ${nume},</p>
      <p>Votre profil partenaire My Darrin est désormais complet à 100 %. Vous pouvez maintenant recevoir des commandes attribuées via la plateforme.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Aller au tableau de bord</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.fr}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.fr}</p>
    `,
  },
  de: {
    subiect: () => 'Dein Partnerprofil ist vollständig! 🎉',
    corp: (nume) => `
      <p>Herzlichen Glückwunsch ${nume},</p>
      <p>Dein My Darrin Partnerprofil ist jetzt zu 100 % vollständig. Du kannst jetzt über die Plattform zugewiesene Aufträge erhalten.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Zum Dashboard</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.de}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.de}</p>
    `,
  ...
  es: {
    subiect: () => '¡Tu perfil de socio está completo! 🎉',
    corp: (nume) => `
      <p>Felicidades ${nume},</p>
      <p>Tu perfil de socio My Darrin ahora está completo al 100%. Ya puedes recibir pedidos asignados a través de la plataforma.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Ir al panel</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.es}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.es}</p>
    `,
  },
  tr: {
    subiect: () => 'Partner profiliniz tamamlandı! 🎉',
    corp: (nume) => `
      <p>Tebrikler ${nume},</p>
      <p>My Darrin partner profiliniz artık %100 tamamlandı. Artık platform üzerinden atanan siparişleri alabilirsiniz.</p>
      <p><a href="${DASHBOARD_PARTENER_URL}" style="${BTN_STYLE}">Panele git</a></p>
      <p style="font-size:12px;color:#666">${GDPR_FOOTER_SCURT.tr}</p>
      <p style="font-size:12px;color:#666">${OPERATOR_FOOTER.tr}</p>
    `,
  },
...
function _numeAfisatPartenerGeneric(nume, l) {
  return nume || FALLBACK_NUME_PARTENER[l] || FALLBACK_NUME_PARTENER[LIMBA_FALLBACK];
}

function renderEmailReminderProfilIncomplet(limba, { nume }) {
  const l = limbaValida(limba);
  const set = REMINDER_PROFIL_INCOMPLET[l] || REMINDER_PROFIL_INCOMPLET[LIMBA_FALLBACK];
  const numeAfisat = _numeAfisatPartenerGeneric(nume, l);
  return { subiect: set.subiect(), html: LOGO_HTML_EMAIL + set.corp(numeAfisat) };
}

function renderEmailProfilPartenerComplet(limba, { nume }) {
  const l = limbaValida(limba);
  const set = PROFIL_PARTENER_COMPLET[l] || PROFIL_PARTENER_COMPLET[LIMBA_FALLBACK];
  const numeAfisat = _numeAfisatPartenerGeneric(nume, l);
  return { subiect: set.subiect(), html: LOGO_HTML_EMAIL + set.corp(numeAfisat) };
}
...
module.exports = {
  ...
  renderEmailComandaConfirmata,
  renderEmailCodPartenerServicii,
  renderEmailCodPartenerCurier,
  renderEmailCodPartenerInchiriere,
  renderEmailReminderProfilIncomplet,
  renderEmailProfilPartenerComplet,
  textQrCaptionCod,
};
